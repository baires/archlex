...
// Official Kubernetes artwork is authored in Inkscape: documents carry RDF
// <metadata>, <sodipodi:namedview> bookkeeping, foreign-namespace attributes
// (inkscape:*, sodipodi:*), and inline `style` attributes instead of
// presentation attributes. These are normalized away before sanitizing.
// Labeled control-plane variants additionally embed <text> labels; the
// renderer draws its own labels, so embedded label text is dropped.
const FOREIGN_CONTAINER_ELEMENTS = new Set(["metadata", "namedview", "text"]);
...
// Rewrites an inline `style` attribute into plain presentation attributes.
// Only paint properties the sanitizer models are kept; typographic and
// bookkeeping properties are dropped. Content-hiding values are rejected so
// artwork never disappears silently.
function inlineStyleAttribute(element, sourceName) {
  ...
  for (const declaration of style.split(";")) {
    ...
    if (property === "display" && value !== "inline") {
      failSvg(sourceName, `content-hiding display value ${value}`);
    }
    if (
      property === "visibility" &&
      value !== "visible" &&
      ...
      failSvg(sourceName, `content-hiding visibility value ${value}`);
    }
    ...
    if (!INLINABLE_PROPERTIES.has(property)) continue;
...
// Strips Inkscape authoring artifacts so only portable SVG remains:
// foreign-namespace containers (<metadata>, <sodipodi:namedview>, RDF),
// foreign-namespace attributes (inkscape:*, sodipodi:*), and inline styles.
function normalizeInkscapeSvg(document, sourceName) {
  const root = document.documentElement;

  // Inkscape hyperlink wrappers (<a>) carry no rendering meaning here;
  // unwrap them so their artwork survives.
  ...
  for (const element of allDescendants(root)) {
    ...
    if (
      FOREIGN_CONTAINER_ELEMENTS.has(element.localName) ||
      (element.prefix !== null && namespace !== XLINK_NAMESPACE) ||
      (namespace !== null &&
  ...
  for (const element of allDescendants(root)) {
    ...
    for (let i = 0; i < element.attributes.length; i += 1) {
      ...
      // Inert typographic/interactivity leftovers on path art (labels were
      // dropped above; pointer interaction is irrelevant in static output).
      if (
        attribute.localName === "text-anchor" ||
        attribute.localName === "pointer-events"
      ) {
        foreignAttributes.push(attribute.name);
        continue;
      }
      if (
    ...
    for (const name of foreignAttributes) element.removeAttribute(name);

    inlineStyleAttribute(element, sourceName);
  }
}
...
function serializeSafeSvg(document, sourceName) {
  ...
  function serializeElement(element, isRoot = false) {
    ...
    for (let index = 0; index < element.attributes.length; index += 1) {
      ...
      const attributeNamespace = attribute.namespaceURI ?? null;
      // Namespace declarations are inert: normalizeInkscapeSvg has already
      // removed every prefixed element/attribute, so they are simply dropped.
...
// Icons retrieved from the Kubernetes community icon set
// (https://github.com/kubernetes/community/tree/main/icons), pinned to
// commit 43d6605709182dedb495a864930ece08666a1e67. Licensed under a choice of
// Apache-2.0 or CC-BY-4.0. Local files are named by catalog id.
const officialIconEntries = [
  ...
  sourcePath: fileURLToPath(
    new URL(`../assets/official/${id}.svg`, import.meta.url),
  ),
}));

export function sanitizeK8sSvg(svg, sourceName) {
  const document = parseSvgDocument(svg, sourceName);
  const root = document.documentElement;
  normalizeInkscapeSvg(document, sourceName);
  inlineK8sSvgStyles(document, sourceName);
    ...
    [...entries]
      ...
      .map(async ({ key, sourcePath }) => {
        try {
          ...
          const sanitized = sanitizeK8sSvg(source, sourcePath);
...
async function main() {
  ...
  if (process.argv.includes("--check")) {
    ...
    if (current !== generated) {
      throw new Error(
        "generated K8S icon module is out of date; run pnpm icons:generate",
      );
    }
    return;
  }
