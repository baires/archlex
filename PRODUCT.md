...
- The core library stays framework-neutral; React exists only in the playground, which consumes only public `@cloudmer/core` behavior.
- Strict TypeScript, ESM-only monorepo (pnpm workspaces + Turborepo); packages remain private and the `@cloudmer/*` npm scope is provisional until the release phase.
...
- Approved product intent: `docs/superpowers/specs/2026-07-27-cloudmer-foundation-design.md`.
...
1. **Semantics are the product.** CloudMer wins by understanding cloud architecture, not by drawing prettier boxes.
